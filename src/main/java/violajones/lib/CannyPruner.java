...
//TODO: WB replace this alltogether!!
	...
	public static int[][] getIntegralCanny(int[][] grayImage) {
		...
		for (int i = 1; i < canny.length - 1; i++)
			for (int j = 1; j < canny[0].length - 1; j++) {
				int grad_x = -canny[i - 1][j - 1] + canny[i + 1][j - 1] - 2
						* canny[i - 1][j] + 2 * canny[i + 1][j]
						- canny[i - 1][j + 1] + canny[i + 1][j + 1];
				int grad_y = canny[i - 1][j - 1] + 2 * canny[i][j - 1]
						+ canny[i + 1][j - 1] - canny[i - 1][j + 1] - 2
						* canny[i][j + 1] - canny[i + 1][j + 1];
				grad[i][j] = Math.abs(grad_x) + Math.abs(grad_y);
			}

		/*
		 * Suppression of non-maxima of the gradient and computation of the
		 * integral Canny image.
		 * WB: where is the nonmax-summpression??
		 */
