...
public class HaarCascadeDescriptor2 {
	...
	private void buildFrom(Document xmlDoc) throws Exception {
		...
		Element root = (Element) xmlDoc.getDocumentElement().getChildNodes().item(0);	//new2
		...
		String type_id = root.getAttribute("type_id");	//new2
		...
		Scanner scanner = new Scanner(root.getElementsByTagName("size").item(0).getTextContent()); //new2
